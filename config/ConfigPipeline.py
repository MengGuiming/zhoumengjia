...
class ConfigPipeline(object):
    ...
    def load_train_data(self):
        ...
        self.data_train_node_sent_index = np.load(os.path.join(self.data_path, prefix+'_node_sent_index.npy'))
        ...
        self.data_train_sdp_sent_index = np.load(os.path.join(self.data_path, prefix+'_sdp_sent_index.npy'))
    ...
    def load_test_data(self):
        ...
        self.data_test_node_sent_index = np.load(os.path.join(self.data_path, prefix+'_node_sent_index.npy'))
        ...
        self.data_test_sdp_sent_index = np.load(os.path.join(self.data_path, prefix+'_sdp_sent_index.npy'))
    ...
    def get_train_batch(self):
        ...
        # evidence
        h_t_sentence_mask = torch.Tensor(self.batch_size, self.h_t_limit, self.sent_limit).cuda()
        h_t_node_mask = torch.Tensor(self.batch_size, self.h_t_limit, self.max_node_num).cuda()
        h_t_sdp_mask = torch.Tensor(self.batch_size, self.h_t_limit, self.max_entity_num).cuda()
        ...
        node_sent_index = torch.zeros(self.batch_size, self.max_node_num, self.sent_limit).float().cuda()
        ...
        sdp_sent_index = torch.zeros(self.batch_size, self.max_entity_num, self.sent_limit).float().cuda()
        ...
        for b in range(self.train_batches):
            ...
            for mapping in [h_mapping, t_mapping, h_t_sentence_mask, h_t_node_mask, h_t_sdp_mask]:
                mapping.zero_()
            ...
            for i, index in enumerate(cur_batch):
                ...
                for label in labels:
                    ...
                    evi2label[(label['h'], label['t'])] += label['evidence']
                ...
                node_sent_index[i].copy_(torch.from_numpy(self.data_train_node_sent_index[index]))
                ...
                sdp_sent_index[i].copy_(torch.from_numpy(self.data_train_sdp_sent_index[index]))
                ...
                max_sents = max(max_sents, len(ins['Ls']) - 1)
                ...
                for j, (h_idx, t_idx) in enumerate(train_tripe):
                    ...
                    for evi in evidence:
                        h_t_sentence_mask[i, j, evi] = 1
                        for m_i, m_node in enumerate(node_sent_index[i].unbind()):
                            if len(m_node.nonzero()) == 0:
                                break
                            m_sentence = int(m_node.nonzero())
                            if m_sentence == evi:
                                h_t_node_mask[i, j, m_i] = 1
                        for s_i, s_node in enumerate(sdp_sent_index[i].unbind()):
                            if len(s_node.nonzero()) == 0:
                                break
                            s_sentence = int(s_node.nonzero())
                            if s_sentence == evi:
                                h_t_sdp_mask[i, j, s_i] = 1
                ...
                for j, (h_idx, t_idx) in enumerate(ins['na_triple'][:lower_bound], len(train_tripe)):
                    ...
                    # 匹配supporting sentences
                    evidences = []

                    for h in hlist:
                        h_mapping[i, j, h['pos'][0]:h['pos'][1]] = 1.0 / len(hlist) / (h['pos'][1] - h['pos'][0])
                        if h['sent_id'] not in evidences:
                            evidences.append(h['sent_id'])

                    for t in tlist:
                        t_mapping[i, j, t['pos'][0]:t['pos'][1]] = 1.0 / len(tlist) / (t['pos'][1] - t['pos'][0])
                        if t['sent_id'] not in evidences:
                            evidences.append(t['sent_id'])
            ...
            yield {'context_idxs': context_idxs[:cur_bsz, :max_c_len].contiguous(),
                   ...
                   'node_sent_index':node_sent_index[:cur_bsz, :b_max_mention_num, :max_sents].contiguous(),
                   ...
                   'sdp_sent_index': sdp_sent_index[:cur_bsz, :max_sdp_num, :max_sents].contiguous(),
                   'sdp_num': sdp_nums,
                   'h_t_sentence_mask':h_t_sentence_mask[:cur_bsz, :max_h_t_cnt, :max_sents].contiguous(),
                   'h_t_node_mask':h_t_node_mask[:cur_bsz, :max_h_t_cnt, :b_max_mention_num].contiguous(),
                   'h_t_sdp_mask':h_t_sdp_mask[:cur_bsz, :max_h_t_cnt, :max_sdp_num].contiguous()
                   }
    ...
    def get_test_batch(self):
        ...
        for b in range(self.test_batches):
            ...
            for i, index in enumerate(cur_batch):
                ...
                node_sent_index[i].copy_(torch.from_numpy(self.data_train_node_position_sent[index]))
                ...
                for h_idx in range(L):
                    for t_idx in range(L):
                        if h_idx != t_idx:
                            ...
                            if (h_idx, t_idx) in evi2label.keys():
                                evidence = evi2label[(h_idx, t_idx)]
                                for evi in evidence:
                                    h_t_sentence_mask[i, j, evi] = 1
                                    for m_i, m_node in enumerate(node_sent_index[i].unbind()):
    ...
    def train(self, model_pattern, model_name):
        ...
        for epoch in range(self.max_epoch):
            ...
            for no, data in enumerate(self.get_train_batch()):
                ...
                node_sent_index = data['node_sent_index'].cuda()
                sdp_sent_index = data['sdp_sent_index'].cuda()
                h_t_sentence_mask = data['h_t_sentence_mask'].cuda()
                h_t_node_mask = data['h_t_node_mask'].cuda()
                h_t_sdp_mask = data['h_t_sdp_mask'].cuda()
                ...
                predict_re = model(context_idxs, context_pos, context_ner,
                                   ...
                                   node_sent_index, sdp_sent_index, h_t_sentence_mask, h_t_node_mask, h_t_sdp_mask)
