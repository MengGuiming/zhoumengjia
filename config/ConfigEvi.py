...
class ConfigEvi(object):
    def __init__(self, args):
        ...
        self.evi_loss = nn.MarginRankingLoss(margin=0.5)
    ...
    def get_train_batch(self):
        ...
        # evidence sentences
        evidence_label = torch.Tensor(self.batch_size, self.h_t_limit, self.sent_limit).cuda()
        evidence_mask = torch.Tensor(self.batch_size, self.h_t_limit, self.sent_limit).cuda()
        ...
        for b in range(self.train_batches):
            ...
            for mapping in [relation_multi_label, relation_mask, pos_idx, evidence_label, evidence_mask]:
                mapping.zero_()
            ...
            yield {'context_idxs': context_idxs[:cur_bsz, :max_c_len].contiguous(),
                   ...
                   # 新增evidence
                   'evidence_label': evidence_label[:cur_bsz, :max_h_t_cnt, :max_sents].contiguous(),
                   'evidence_mask': evidence_mask[:cur_bsz, :max_h_t_cnt, :max_sents],
    ...
    def train(self, model_pattern, model_name):
        ...
        for epoch in range(self.max_epoch):
            ...
            for no, data in enumerate(self.get_train_batch()):
                ...
                evidence_mask = data['evidence_mask']
                ...
                predict_re, sent_scores = model(context_idxs, context_pos, context_ner, input_lengths,
                ...
                loss_re = torch.sum(BCE(predict_re, relation_multi_label)*relation_mask.unsqueeze(2)) / torch.sum(relation_mask)
                evidence_mask = torch.ByteTensor(evidence_mask.cpu() > 0).cuda()
                sent_scores = torch.masked_select(sent_scores, evidence_mask)
                evidence_label = torch.masked_select(evidence_label, evidence_mask)
                pos_mask = torch.ByteTensor(evidence_label.cpu()>0).cuda()
                neg_mask = torch.ByteTensor(evidence_label.cpu()<1).cuda()
                sent_scores_pos = torch.masked_select(sent_scores, pos_mask)
                sent_scores_neg = torch.masked_select(sent_scores, neg_mask)
                len_pos = sent_scores_pos.shape[0]
                len_neg = sent_scores_neg.shape[0]
                sent_scores_pos = sent_scores_pos.unsqueeze(0).repeat(len_neg, 1).view(-1)
                sent_scores_neg = sent_scores_neg.unsqueeze(1).repeat(1, len_pos).view(-1)
                loss_evi = self.evi_loss(sent_scores_pos.unsqueeze(0), sent_scores_neg.unsqueeze(0), torch.ones(1).cuda())
                loss = loss_re + loss_evi
                # print(loss_re.data, loss_evi.data, loss.data)
