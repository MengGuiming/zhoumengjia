...
class LSRPIPELINE(nn.Module):
    ...
    def forward(self, context_idxs, pos, context_ner, h_mapping, t_mapping,
        ...
        docs_rep, _ = self.doc_encoder(sent_emb, context_seg)
        ...
        for batch_no, (m_n, e_n, s_n) in enumerate(zip(mention_num_list, entity_num_list.long().tolist(), sdp_num_list)):
            ...
            gcn_inputs = []
            max_mention_h_t_num = 0
            mention_node_position_h_t = []
            for h_t_no, (m_mask, s_mask) in enumerate(zip(h_t_node_mask[batch_no], h_t_sdp_mask[batch_no])):
                m_index = m_mask.nonzero().unbind(dim=1)
                s_index = s_mask.nonzero().unbind(dim=1)
                max_mention_h_t_num = max(max_mention_h_t_num, m_index[0].shape[0])
                mention_node_position_h_t.append(mention_node_position[batch_no][m_index])
                gcn_inputs.append(torch.cat((m_rep[m_index], e_rep, s_rep[s_index]),dim=0))

            output = pad_sequence(gcn_inputs).permute(1, 0, 2)
            del gcn_inputs
            ...
            mention_node_position_h_t = pad_sequence(mention_node_position_h_t).permute(1,2,0)
            output = torch.bmm(mention_node_position_h_t[:, :max_doc_len, :max_mention_h_t_num], output[:, :max_mention_h_t_num])
            context_output_ht = torch.add(context_output[batch_no].unsqueeze(0).repeat(h_t_node_mask.shape[1],1,1), output)
            del output, mention_node_position_h_t
            start_re_output.append(torch.matmul(h_mapping[batch_no].unsqueeze(2).permute(0,2,1), context_output_ht).squeeze(1))
            end_re_output.append(torch.matmul(t_mapping[batch_no].unsqueeze(2).permute(0,2,1), context_output_ht).squeeze(1))
            del context_output_ht
        ...
        start_re_output = pad_sequence(start_re_output).permute(1,0,2)
        end_re_output = pad_sequence(end_re_output).permute(1,0,2)

        s_rep = torch.cat([start_re_output, self.dis_embed(dis_h_2_t)], dim=-1)
        t_rep = torch.cat([end_re_output, self.dis_embed(dis_t_2_h)], dim=-1)
        del start_re_output, end_re_output
